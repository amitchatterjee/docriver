# Create a key pair + x509 certificate for docriver. By default, it will be stored in $HOME/.ssh directory
...
token=$(python $DOCRIVER_GW_HOME/server/token_issue.py --keystore $HOME/.ssh/p123456.p12  --password docriver --resource document --expires 300 --subject someoneabc --permissions realm:p123456 resourceType:claims documentCount:10)

echo $token

# Authorize the token using docriver keystore
python $DOCRIVER_GW_HOME/server/token_decode.py --keystore $HOME/.ssh/docriver.p12 --password docriver --token "$token"
